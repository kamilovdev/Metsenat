  ...
  <div class="containerMain">
    <div class="flex justify-between my-5">
      ...
      <div class="flex">
          ...
          <div v-if="limitshowopen" class="absolute top-6 text-center">
            <div class=" show-limit">
              <ul class="flex flex-col gap-2 bg-white  rounded-[4px]" v-for="limit in limitPageSize" :key="limit.id">
                <li class="border-t border-b px-3 hover:border-[#3366FF] cursor-pointer transition-all duration-300"
                  @click="$emit('onChangePageSize', limit.num)">{{ limit.num }}</li>
              </ul>
            </div>
          </div>
        </div>
        ...
        <div class="flex">
          <button @click="$emit('onChangePageMinus', pageNumber)"
            class="rotate-90 text-[12px] font-normal text-[#979797] bg-[#FFFFFF] px-[5px] mx-1 w-fit border border-[#DFE3E8] rounded-[4px] flex items-center gap-[8px]">
            <i class="icon-angle-down"></i>
          </button>
          <div v-for="page in totalPages" :key="page" class="flex gap-3" @click="$emit('onChangePage', page)"> 
            <button :class="{'bg-[#3366FF] text-white': page ===   pageNumber}" 
              class="text-[15px] font-normal text-[#1D1D1F] bg-[#FFFFFF] px-2 mx-1 w-fit border border-[#DFE3E8] rounded-[4px] flex items-center gap-[8px]"
              v-if="(Math.abs(page - pageNumber) <= 3) || (page <= 5)">
              {{ page }}
            </button>
          </div>
          <button @click="$emit('onChangePagePlus', pageNumber)"
            class="-rotate-90 text-[12px] font-normal text-[#979797] bg-[#FFFFFF] px-[5px] mx-1 w-fit border border-[#DFE3E8] rounded-[4px] flex items-center gap-[8px]">
            <i class="icon-angle-down"></i>
          </button>
        </div>
